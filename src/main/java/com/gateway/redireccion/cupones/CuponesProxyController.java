...
public class CuponesProxyController {
    ...
    public ResponseEntity<?> proxyCupones(HttpServletRequest request,
        ...
        String originalPath = request.getRequestURI().substring(
                request.getRequestURI().indexOf("/api/proxy/cupones") + "/api/proxy/cupones".length()
        );

        String targetUrl = "http://localhost:8082/api/cupones" + originalPath;
